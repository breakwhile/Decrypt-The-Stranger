...
public class GameActivity extends Activity {
	...
	private int socer = 0;
	private final int GAME_RESULT = 120;
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		btnrestart.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("gameResult", "fail");
				// 设置该SelectActivity的结果码，并设置结束之后退回的Activity
				setResult(GAME_RESULT, intent);
				finish();
			}
		});
    }
	...
	public void overGame(){
       ...
       if(total==7&&number[8]==0){
		   Toast.makeText(GameActivity.this, "解密成功，允许查看Ta的信息:)", 0).show();
		   for(int i=0;i<9;i++){
	        	number[i]=DataHelper.number[i];
	        }
		   socer = 0 ;
		   tv_socer.setText("移动了 "+String.valueOf(socer)+" 步");
		   Intent intent = getIntent();
		   intent.putExtra("gameResult", "success");
		   // 设置该SelectActivity的结果码，并设置结束之后退回的Activity
		   this.setResult(GAME_RESULT, intent);
